<template>
    <div id="serviceOrder" style="position:relative;background:#efeff4">
        ...
        <div class="item" style="background:#fff;margin-top:.2rem;" v-if="mainContentShow">
            <span class="transB time_from_medical"></span>医疗部提交时间:{{submitTime}}
        </div>
    </div>
</template>
<script>
    ...
    methods:{
        FetchData(url,QueryData){
            ...
            this.$http.post(url,QueryData).then((res)=>{
                        ...
                        _this.submitTime=res.data.up_finishtime//医疗部提交时间
                        ...
                        differPrice[3].money=Number(result.cash_pay);//现金支付
                        differPrice[4].money=Number(result.pos_pay);//POS机支付
                        differPrice[5].money=Number(result.shop_recPay);//店家代收款
                        differPrice[6].money=Number(result.beforeEarnest);//以往定金转入
                        //<...................设计部模块...................>
